...
function contarOvejas(ovejas){
    const ovejas_color_rojo = []
    ovejas.forEach(element => {
        console.log(element.name)
        let nombre_oveja = element.name.toLowerCase();
        if(element.color == 'rojo' && nombre_oveja.includes('n') && nombre_oveja.includes('a')){
            ovejas_color_rojo.push(element)
        }
    });
    return ovejas_color_rojo
}
